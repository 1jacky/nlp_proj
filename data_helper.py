    ...
    def encoder(self, data_path):
        ...
        with open(data_path,encoding='utf8') as f:
            ...
            for line in lines:
                ...
                tmp_lable = [0] * self.class_num
                for label in line["label"]:
                    tmp_lable[self.label2idx[label]]=1
                labels.append(tmp_lable)

        tokenizers = self.tokenizer(texts,padding=True,
        ...
        return input_ids,token_type_ids,attention_mask,torch.tensor(labels,dtype=torch.float)

    def __len__(self):
        return len(self.labels)

    def __getitem__(self, item):
        return self.input_ids[item],self.attention_mask[item], self.token_type_ids[item], self.labels[item]
